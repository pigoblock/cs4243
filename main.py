...
def processInput():
    ...
    for i in range(len(array_points_per_plane)):
        ...
        print planeDirection
        if numPoints == 4:
            resultImg = generate3DpointsRectangle(numPoints, planeDirection, resultImg)
    cv2.imwrite("big bang.jpg", resultImg)
...
def generate3DpointsRectangle(numPoints, planeDirection, resultImg):
    ...
    resultPoints = getProjectedPoints(np.array(array_points_3d, np.float32), (0,0,0), (0,5,0), camera, 0)
    ...
    elif planeDirection == 'down':
        cv2.fillConvexPoly(resultPicture, polygon, [227, 191, 145])
    else:
        cv2.fillConvexPoly(resultPicture, polygon, [141,175,204])

    cv2.imwrite("LastPart.jpg", resultPicture)
    '''
    ...
    for j in range(scene_height):
        for i in range(scene_width):
            ...
            if planeDirection == 'front':
                ...
                if j == 0 and i == 0:
                    print array_3d_points_with_color[len(array_3d_points_with_color)-1]
...
for i in range(850):
    for j in range(1632):
        resultPicture[i][j] = [227, 191, 145]

for i in range(850, 1224):
    for j in range(1632):
        resultPicture[i][j] = [39, 92, 66]
